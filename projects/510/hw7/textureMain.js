...
function setUpTextures(){
    ...
    // load the actual image
    var sceneImage = document.getElementById ('scene-texture')
    sceneImage.crossOrigin = "";

    // bind the texture so we can perform operations on it
    gl.bindTexture (gl.TEXTURE_2D, sceneTexture);

    // load the texture data
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, sceneImage.width, sceneImage.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, sceneImage);
...
function drawCurrentShape () {
	...
	gl.uniform1i(program.proc, false);
    }
	...
	gl.uniform1i(program.proc, true);
    }
...
function initProgram (vertexid, fragmentid) {
  ...
  // toggling the procedural texture on/off	
  program.proc = gl.getUniformLocation(program, 'proc'); 
